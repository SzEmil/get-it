...
function replaceVATInvoicePlaceholders(html: string, invoice: Invoice): string {
...
export const generateVATInvoicePdfLib = async (
  invoice: Invoice
): Promise<Buffer> => {
  const currentDate = new Date().toISOString().split('T')[0];
  const exchange_rate = invoice.exchange_rate ?? 1;
  ...
  const cell5Width =
    colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4];
  const rightTextX = tableStartX + cell5Width - 50; // od prawej
